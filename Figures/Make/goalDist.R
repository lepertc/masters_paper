...
p2 <- ggplot(df, aes(x = vgoal)) + geom_histogram(binwidth = 1) + 
  labs(title = "Visitor goals",
       y = NULL, x = "Number of goals") +
  geom_text(aes(x = 6, y = 1500, label = v_lab)) +
  scale_x_continuous(breaks = c(0, 2, 4, 6, 8), labels = c(0, 2, 4, 6, 8)) +
  geom_point(data = df_v, aes(x = n_goal, y = count))

p <- grid.arrange(p1, p2, top = "Distribution of goals overlayed with expected number of goals assuming a poisson distribution")
